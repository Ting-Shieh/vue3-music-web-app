/*
 * 該文件是運行在 Node.js 端的，獲取數據的基本思路就是後端代理，
 * 即提供接口路由供前端葉面使用，然後在路由內部，我們接收到前端請求後，再發送 HTTP 請求到第三方服務接口，
 * 攜帶相對應的請求參數，包含簽名的參數等等..。
 * 對於從第三方api返回的數據，我們會做一層數據處理，
 * 最終提供給前端的數據，前端可以直接使用，無須再處理。
 * 這樣也比較符合真實企業項目的開發規範，即數據處理放在後端，前端只做數據渲染和交互。
 */
...
function registerSongsUrl(app) {
  app.get('/api/getSongsUrl', (req, res) => {
    ...
    let midGroup = []
    // 第三方接口只支持最多處理 100 條數據，所以如果超過 100 條數據，我们要把數據按每組 100 條切割，發送多個請求
    if (mid.length > 100) {
